...
### Schedule 服務（定時報告）
- `/schedule <開課序號> <學年> <學期> [間隔分鐘] [頻道ID]` - 設定定時課程報告
  - 設定後立即發送首次報告
  - 預設每 60 分鐘回報一次
  - 可指定頻道接收報告，未指定則發送私訊
...
/schedule 1001 114 1 30    # 每 30 分鐘報告
...
### 注意事項
開課序號可在台師大選課系統的課程列表中找到，每個開課班級都有唯一的序號。Track 監控** ([monitor.js](monitor.js)) - 定期檢查已訂閱課程狀態變化並發送通知（僅處理有 lastFull 屬性的訂閱）
